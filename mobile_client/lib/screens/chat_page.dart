...
| **能力** | 生成最终回答给用户 |
| **输出** | 自然语言回复展示给用户 |
| **失败** | 不会失败，但回答质量取决于之前收集的信息 |
| **边界** | ⚠️ 应在收集足够信息后使用，简单问候除外 |
...
- **知识库非空 + 出现"补充/扩展/完善/教程/总结/整理/改写/优化/修改/这个文档/这份文件/刚才的文件"** → 必须先 search_knowledge 检索相关内容，再 read_knowledge 获取完整段落，最后综合输出；如需保存则 save_file。
